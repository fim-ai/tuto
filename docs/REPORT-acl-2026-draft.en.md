...
2. **The defects are on the second promise, support, but the size of the problem does not hold still.** In the audited sample of 100 ACL 2026 papers, 16% [10-24] contained at least one confirmed problem citation and the citation-level rate was 0.95% [0.6-1.5] (20 of 2,110). Two independent re-draws through the same pipeline returned 5.66% and 6.12%, pooling to 5.90% [5.12, 6.80]. The measurement does not reproduce (see the top note); we report all three and treat the non-replication as the headline finding.
3. **Peer review barely touches citation quality.** Accepted papers and unreviewed preprints show nearly the same confirmed-problem rate (0.95% vs. 1.2%, intervals overlapping heavily). This is not a scandal; it confirms what everyone already knew. Reviewers evaluate claims and experiments, not reference lists. This layer of checking simply did not exist before. (This comparison rests on the L2 support layer that does not reproduce, per the top note; read it as provisional.)
4. **The number-one enemy of automated citation auditing is its own false positives, not its misses.** In the audited sample, 130 first-pass flags survived arbitration as only 20 confirmed findings: first-pass precision 15.4% [10.3-22.4] (14.6% pooled with the pilot, 25/171). Any "AI citation checker" that does not publish its own false-positive rate should be presumed untrustworthy. We publish our two-stage architecture, our error taxonomy, and every precision number, together with the code.
...
flags (partial / not_supported, ~5-7%)
   ...
   ▼
confirmed (~15% of first-pass flags)
```
...
> The numbers in this section come from the audited and pilot runs of the L2 layer, which does not reproduce (see the 2026-07-21 update). They are kept as the audited view; the "restrained" framing and every rate here are superseded by the three-run picture at the top, where the pooled estimate is 5.90%.
...
- **Against four decades of hand-checked medical citation studies**: the pooled quotation-error rate there is 16.9%. Aligning definitions (drop minor errors, drop secondary quotations, convert per-reference to per-citation ÷1.66, exclude nominal citations ×1.22) brings that baseline to a comparator near 3%. The audited 0.95% sits about 3x below it; the pooled 5.90% sits about 2x above. Which side of the historical baseline modern NLP falls on depends entirely on which run one credits: the residual flips sign, the sharpest illustration of the non-replication.

`unverifiable_from_text` (26-33%) is a method ceiling, not a finding: the cited paper has no arXiv full text (coverage is about 83%) or passage retrieval missed the claim's location. The bucket is reported as-is and never counted toward any error rate.

## 7. Finding three: the false-positive problem of automated citation auditing (methodological contribution)

> First-pass precision is itself a property of the non-reproducing layer: in the two later draws the arbiter confirmed `not_supported` flags at 61-67% against the audit's 16%, so the ~15% first-pass precision (20/130) below is the audited-run value and does not hold across re-runs. The qualitative point (a checker must publish its false-positive rate) stands; the specific number does not.
...
| Judge error / insufficient evidence | 3 | The rest |
...
| First-pass precision this round | 20/130 = 15.4% (pilot 5/41 = 12%) |
...
2. **arXiv full-text coverage is about 83%**: non-arXiv references degrade to abstract-based judgment, and anything an abstract cannot settle counts as unverifiable.
